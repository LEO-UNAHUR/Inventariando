...
import { Sun, Moon, ShoppingBag, TrendingUp, Calendar, ArrowRight, DollarSign } from 'lucide-react';
    ...
    <div className="h-full flex flex-col pb-20 bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
      ...
      <header className="sticky top-0 bg-slate-50 dark:bg-slate-950 z-10 p-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center transition-colors">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm">Registro y Punto de Venta</p>
        </div>
        <button 
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        <div className="space-y-3">
             <h3 className="font-semibold text-slate-800 dark:text-slate-200 px-1">Más Vendidos (Mes)</h3>
             {stats.bestSellers.length === 0 ? (
                 ...
                 stats.bestSellers.map((item, idx) => (
                    <div key={idx} className="bg-white dark:bg-slate-900 p-3 rounded-xl border border-slate-100 dark:border-slate-800 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${idx === 0 ? 'bg-yellow-100 text-yellow-700' : 'bg-slate-100 text-slate-500 dark:bg-slate-800'}`}>
                                {idx + 1}
                            </span>
                            <div>
        ...
        <div className="space-y-3 pt-2">
            <h3 className="font-semibold text-slate-800 dark:text-slate-200 px-1">Últimas Transacciones</h3>
            {sales.slice().reverse().slice(0, 5).map(sale => (
                <div key={sale.id} className="bg-white dark:bg-slate-900 p-3 rounded-xl border border-slate-100 dark:border-slate-800 flex justify-between items-center">
                    <div>
                        <p className="text-sm font-medium text-slate-800 dark:text-slate-100">Venta #{sale.id.slice(0,4)}</p>
                    ...
                    <div className="text-right">
                        ...
                        <p className="text-[10px] text-slate-400 uppercase">{sale.paymentMethod || 'Efectivo'}</p>
                    </div>
                </div>
            ))}
        </div>
