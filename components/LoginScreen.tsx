...
const LoginScreen: React.FC<LoginScreenProps> = ({ users, onLogin, isDark }) => {
  ...
  const getRoleColor = (role: Role) => {
    switch (role) {
      case Role.ADMIN: return 'bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-300';
      case Role.MANAGER: return 'bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300';
      case Role.SELLER: return 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300';
                     ...
                     >
                         <div className="flex items-center gap-4">
                             ...
                             <div>
                                 ...
                                 <span className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${getRoleColor(user.role)}`}>
                                     {user.role}
                                 </span>
                             </div>
                         </div>
        ...
        {step === 'PASSWORD' && selectedUser && (
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl p-6 border border-slate-200 dark:border-slate-800 animate-slide-left">
              <div className="flex items-center gap-3 mb-6">
                  <button onClick={() => setStep('SELECT_USER')} className="text-sm text-slate-400 hover:text-blue-500">
                      ← Cambiar
                  </button>
                  <div className="flex-1 text-center pr-8">
                       <h3 className="font-bold text-slate-800 dark:text-slate-100">{selectedUser.name}</h3>
        ...
        {step === '2FA' && selectedUser && (
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl p-6 border border-slate-200 dark:border-slate-800 animate-slide-left">
                <div className="text-center mb-6">
                    ...
                    <h3 className="font-bold text-slate-800 dark:text-slate-100">Verificación de 2 Pasos</h3>
