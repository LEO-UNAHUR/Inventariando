...
const UserProfile: React.FC<UserProfileProps> = ({ user, onUpdateUser, isDark }) => {
  ...
  const handleSendVerificationCode = () => {
    ...
    window.open(
      `https://wa.me/${formatted}?text=Tu%20c%C3%B3digo%20de%20verificaci%C3%B3n%20para%20Inventariando%20es:%20${code}`,
      '_blank'
    );
  };
                ...
                <div className="flex items-center justify-between py-2">
                  <div className="flex items-center gap-3">
                    <div
                      className={`p-2 rounded-lg ${
                        is2FA ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-400'
                      }`}
                    >
                      <ShieldCheck size={24} />
                    </div>
                  ...
                  <button
                    ...
                    className={`w-14 h-8 rounded-full p-1 transition-colors duration-300 focus:outline-none ${
                      is2FA ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-slate-700'
                    }`}
                  >
                    <div
          ...
          <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 space-y-4">
            ...
            <div>
              ...
              <p className="text-xs mt-1 text-slate-500">
                Para compartir ventas y productos por WhatsApp
              </p>
            </div>
