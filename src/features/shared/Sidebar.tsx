    ...
    <>
      ...
      <div
        className={`fixed inset-0 bg-black/50 z-[60] transition-opacity duration-300 backdrop-blur-sm ${
          isOpen && !isDesktop ? 'opacity-100' : 'opacity-0 pointer-events-none'
        } ${isDesktop ? 'hidden' : ''}`}
        onClick={onClose}
      />
      ...
      {/* Sidebar Panel */}
      <div
        className={`fixed lg:static top-0 left-0 h-full w-[80%] max-w-xs lg:w-72 bg-white dark:bg-slate-900 z-[70] transform transition-transform duration-300 shadow-2xl lg:shadow-none flex flex-col ${
      ...
      >
        ...
        <div className="p-6 bg-secondary border-b border-border-light flex flex-col gap-4 text-primary">
          <div className="flex justify-between items-start">
            ...
            {!isDesktop && (
              <button
                onClick={onClose}
                className={`p-2 bg-white dark:bg-slate-800 rounded-full shadow-sm text-slate-500 ${
        ...
        <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950">
          ...
          <p className="text-xs text-center text-tertiary mt-3 leading-relaxed px-2">
            {footerText}
          </p>
        </div>
      </div>
