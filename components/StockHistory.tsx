...
const StockHistory: React.FC<StockHistoryProps> = ({ movements, isDark, onToggleTheme }) => {
  ...
  const chartData = useMemo(() => {
    ...
    return last7Days.map(dayLabel => {
      return {
        name: dayLabel,
        Entradas: movements.filter(m => 
          (m.type === MovementType.IN || (m.type === MovementType.ADJUSTMENT && m.quantity > 0)) && 
          new Date(m.date).toLocaleDateString('es-AR', { weekday: 'short' }) === dayLabel
        ).reduce((acc, curr) => acc + Math.abs(curr.quantity), 0),
        Salidas: movements.filter(m => 
          (m.type === MovementType.OUT || (m.type === MovementType.ADJUSTMENT && m.quantity < 0)) && 
          new Date(m.date).toLocaleDateString('es-AR', { weekday: 'short' }) === dayLabel
        ).reduce((acc, curr) => acc + Math.abs(curr.quantity), 0),
      };
    });
  ...
  const handleExport = () => {
    ...
    link.setAttribute("download", `inventariando_movimientos_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    <div className="h-full flex flex-col pb-20 bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 transition-colors">
          <h3 className="text-base font-semibold text-slate-800 dark:text-slate-200 mb-4">Evolución Semanal</h3>
        ...
        <div className="space-y-3">
          <h3 className="text-base font-semibold text-slate-800 dark:text-slate-200 px-1">Últimos Movimientos</h3>
          ...
          {movements.length === 0 ? (
            ...
            movements.slice().reverse().map((movement) => {
              ...
              return (
                <div key={movement.id} className="bg-white dark:bg-slate-900 rounded-xl p-4 border border-slate-100 dark:border-slate-800 shadow-sm flex items-center justify-between transition-colors">
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-full ${isPositive ? 'bg-emerald-100 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400'}`}>
                      {isPositive ? <ArrowUpRight size={20} /> : <ArrowDownLeft size={20} />}
                    </div>
                    <div>
                      <h4 className="font-semibold text-slate-800 dark:text-slate-100 text-sm">{movement.productName}</h4>
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs text-slate-400 uppercase tracking-wider font-medium">{movement.type}</p>
                  </div>
                </div>
              );
            })
