...
const POS: React.FC<POSProps> = ({
  ...
  const [paymentMethod, setPaymentMethod] = useState<
    'EFECTIVO' | 'TRANSFERENCIA' | 'DEBITO' | 'CREDITO' | 'CUENTA_CORRIENTE'
  >('EFECTIVO');
  ...
  const handleCheckout = () => {
    const updatedCustomer = selectedCustomer ? { ...selectedCustomer } : undefined;
    ...
    return (
      <div className="fixed inset-0 bg-base dark:bg-slate-950 z-[60] flex flex-col animate-slide-up">
        <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex items-center gap-2">
          <button onClick={() => setShowCustomerSelect(false)} className="text-secondary">
            <X size={24} />
          </button>
          <input
            type="text"
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-2">
          <button
            ...
            className="w-full p-3 text-left font-medium text-secondary border-b border-slate-100 dark:border-slate-800"
          >
            Consumidor Final
          </button>
          {filteredCustomers.map((c) => (
            <button
              ...
              className="w-full p-3 bg-secondary text-primary border border-slate-100 dark:border-slate-800 rounded-xl flex justify-between items-center"
            >
              <div>
      ...
      <div className="fixed inset-0 bg-base z-50 flex flex-col animate-fade-in">
        ...
        <div className="flex-1 p-6 flex flex-col items-center justify-start space-y-6 overflow-y-auto">
          <div className="text-center w-full">
            ...
            {subtotal > finalTotal && (
              <p className="text-secondary text-sm font-medium mt-1">
                Ahorraste {formatCurrency(subtotal - finalTotal)}
              </p>
            )}
          </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <button
                    ...
                    className={`p-4 rounded-xl border flex items-center justify-center gap-2 font-medium transition-all ${
                      paymentMethod === m.id
                        ? 'bg-blue-600 text-white border-blue-600'
                        : 'bg-secondary border-slate-200 dark:border-slate-700 text-secondary dark:text-secondary'
                    }`}
                  >
                    {m.icon} {m.label}
                  </button>
                ...
                {selectedCustomer && (
                  <button
                    ...
                    className={`col-span-2 p-4 rounded-xl border flex items-center justify-center gap-2 font-medium transition-all ${
                      paymentMethod === 'CUENTA_CORRIENTE'
                        ? 'bg-purple-600 text-white border-purple-600'
                        : 'bg-secondary border-slate-200 dark:border-slate-700 text-purple-600 dark:text-purple-400'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`flex-1 py-1.5 rounded text-xs font-bold border ${
                      fiscalType === type
                        ...
                        : 'bg-secondary dark:bg-slate-900 border-slate-300 dark:border-slate-600 text-secondary'
                    }`}
                  >
                    Factura {type}
                  </button>
            ...
            {paymentMethod === 'EFECTIVO' && (
              <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700 flex gap-4 items-center">
                <input
                  ...
                  className="flex-1 form-control text-lg"
                />
                <div className="text-right">
                  <p className="text-xs text-secondary">Vuelto</p>
                  <p className={`text-xl font-bold ${change < 0 ? 'text-danger' : 'text-success'}`}>
          ...
          <button
            ...
            disabled={paymentMethod === 'EFECTIVO' && cashValue < finalTotal}
            className="w-full max-w-sm bg-blue-600 hover:bg-blue-700 disabled:bg-tertiary disabled:text-secondary text-white py-4 rounded-xl font-bold text-xl shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2"
          >
    ...
    <div className="fixed inset-0 bg-base z-40 flex flex-col md:flex-row animate-slide-up">
      ...
      <div className="flex-1 flex flex-col h-[60%] md:h-full border-b md:border-b-0 md:border-r border-slate-200 dark:border-slate-800">
        <div className="p-4 bg-secondary dark:bg-slate-950 border-b border-slate-100 dark:border-slate-800 flex gap-2 items-center">
          <button onClick={onCancel} className="p-2 mr-2 bg-tertiary rounded-full">
        ...
        <div className="flex-1 overflow-y-auto p-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 content-start">
          {filteredProducts.map((product) => {
            ...
            const hasPromo = promotions.some(
              (pr) => pr.active && pr.targetProductId === product.id
            );
            return (
              <button
                key={product.id}
                onClick={() => addToCart(product)}
                className={`flex flex-col items-start p-3 bg-secondary dark:bg-slate-900 border ${
                  hasPromo
                    ? 'border-pink-300 ring-1 ring-pink-100'
                    : 'border-slate-200 dark:border-slate-800'
                } rounded-xl hover:border-blue-500 transition-colors text-left active:scale-95 relative`}
              >
                {hasPromo && (
                ...
                <p className="font-bold text-blue-600 dark:text-blue-400 mt-auto">
      ...
      <div className="h-[40%] md:h-full md:w-96 flex flex-col bg-secondary dark:bg-slate-900">
        <button
          ...
          className="p-3 bg-secondary border-b border-slate-200 flex justify-between items-center text-primary"
        >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {cart.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-secondary">
              ...
              <p className="text-sm text-secondary">Carrito vacío</p>
            </div>
          ) : (
                ...
                <div className="flex-1">
                  ...
                  {item.appliedPromotion ? (
                    <p className="text-xs text-pink-500 font-medium flex items-center gap-1">
                      <Tag size={10} /> {item.appliedPromotion}
                    </p>
                  ) : (
        ...
        <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-tertiary dark:bg-slate-950">
          ...
          <button
            ...
            className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-slate-300 text-white py-3 rounded-xl font-bold shadow-lg active:scale-95 transition-all flex justify-between px-6 items-center"
          >
            <span>Cobrar</span>
