...
const DataManagement: React.FC<DataManagementProps> = ({
    ...
    reader.onload = (event) => {
      try {
        ...
        if (validProducts.length > 0) {
          ...
          try {
            trackEvent('backup_created', { auto: true });
          } catch {}
          ...
          // Analytics: import
          try {
            trackEvent('data_imported', { format, count: validProducts.length });
          } catch {}
  ...
  const handleClearDatabase = () => {
    ...
    const confirm1 = window.confirm(
      '¡ADVERTENCIA! ¿Estás seguro de que quieres BORRAR TODOS LOS PRODUCTOS?'
    );
    if (confirm1) {
      const confirm2 = window.confirm(
        'Esta acción no se puede deshacer. Se eliminarán todos los productos de la base de datos local. ¿Continuar?'
      );
      if (confirm2) {
  ...
  const handleRollback = (backupId: string) => {
    ...
    const confirm = window.confirm(
      '¿Deseas revertir la base de datos al estado anterior a esta importación? Los cambios realizados después se perderán.'
    );
    if (confirm) {
            ...
            <div className="space-y-2 max-h-40 overflow-y-auto pr-1">
              {logs.length === 0 ? (
                ...
                logs.map((log) => (
                  <div
                    key={log.id}
                    className="flex items-center justify-between p-2 bg-slate-50 dark:bg-slate-800 rounded-lg border border-slate-100 dark:border-slate-700 group"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-xs font-medium text-slate-700 dark:text-slate-200 truncate w-32 sm:w-40">
                          {log.fileName}
                        </p>
                        <p className="text-[10px] text-slate-500">
                          {new Date(log.date).toLocaleString()} • {log.recordCount} items
                        </p>
                      </div>
                    </div>
            ...
            <div className="border-t-2 border-red-100 dark:border-red-900/30 pt-4 mt-2">
              <div className="bg-red-50 dark:bg-red-900/10 p-4 rounded-xl border border-red-100 dark:border-red-900/20">
                ...
                <p className="text-xs text-slate-600 dark:text-slate-400 mb-3">
                  Si tuviste errores al importar o quieres reiniciar el sistema, puedes eliminar
                  toda la base de datos de productos.
                </p>
                <button
