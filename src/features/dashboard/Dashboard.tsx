...
const Dashboard: React.FC<DashboardProps> = ({
  ...
  const handleExportCategory = () => {
    ...
    const fileName = `inventariando_categoria_${selectedCategory.toLowerCase()}_${
  ...
  return (
    <div className="h-full overflow-y-auto no-scrollbar p-4 pb-24 space-y-6 animate-fade-in bg-slate-50 dark:bg-slate-950 transition-colors duration-300 relative">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div
          className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col justify-between transition-colors"
      ...
      <div
        className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 flex items-center justify-between transition-colors"
        style={{ padding: 'var(--spacing-lg)' }}
      >
        <div>
          <p
                ...
                {notifications.length > 0 && (
                  <>
                    ...
                    <button
                      ...
                      className="p-2 text-slate-400 hover:bg-red-100 hover:text-red-500 dark:hover:bg-red-900/20 rounded-full"
                      title="Borrar todo"
                    >
                      <Trash2 size={18} />
                    </button>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-50 dark:bg-slate-950">
              {notifications.length === 0 ? (
                ...
                notifications.map((notif) => {
                  ...
                  if (isRead) {
                    colorClass =
                      'bg-white dark:bg-slate-900 text-slate-500 border-slate-100 dark:border-slate-800 opacity-70';
                  }

                  return (
                    <div
                      key={notif.id}
                      className={`p-3 rounded-xl border ${colorClass} flex gap-3 relative group transition-all`}
                    >
                      {!isRead && (
                      ...
                      <div className="flex-1">
                        <h4 className={`font-semibold text-sm ${isRead ? 'font-normal' : ''}`}>
                          {notif.title}
                        </h4>
