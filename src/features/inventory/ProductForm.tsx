...
const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const [productSuppliers, setProductSuppliers] = useState<ProductSupplierInfo[]>(
    initialProduct?.suppliers || []
  );
  ...
  const handleAddSupplier = () => {
    if (selectedSupplierId && supplierCost) {
      ...
      if (exists) {
        setProductSuppliers(
          productSuppliers.map((ps) => (ps.supplierId === selectedSupplierId ? newInfo : ps))
        );
      } else {
  ...
  const handleAIAssist = async () => {
    ...
    try {
      ...
      const suggestedCat = Object.values(Category).find(
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div className="relative">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
              Usa la varita mágica para sugerir precio y descripción.
            </p>
          </div>
            ...
            {productSuppliers.length > 0 ? (
              <div className="space-y-2">
                {productSuppliers.map((ps, index) => {
                  const sName =
                    suppliers.find((s) => s.id === ps.supplierId)?.name || 'Desconocido';
