...
import { Search, Plus, Phone, Mail, MapPin, Edit2, Trash2, MessageCircle, Sun, Moon, Users, Star, Wallet, History, ArrowRight } from 'lucide-react';
...
const CustomerList: React.FC<CustomerListProps> = ({ customers, sales, onAdd, onEdit, onDelete, isDark, onToggleTheme }) => {
      ...
      return (
          <div className="h-full flex flex-col bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
             <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center gap-3 bg-white dark:bg-slate-900 sticky top-0 z-10">
                 <button onClick={() => setSelectedCustomer(null)} className="p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800">
                     <ArrowRight className="rotate-180" size={20} />
                 </button>
                 <h1 className="font-bold text-lg text-slate-800 dark:text-slate-100">{selectedCustomer.name}</h1>
             </div>
             ...
             <div className="flex-1 overflow-y-auto p-4 space-y-4">
                 ...
                 <div className="grid grid-cols-2 gap-3">
                     ...
                     <div className="bg-gradient-to-br from-amber-400 to-amber-500 rounded-2xl p-4 text-white shadow-lg">
                         ...
                         <p className="text-xs opacity-75 mt-1">Nivel: {selectedCustomer.loyaltyPoints > 1000 ? 'Oro' : 'Plata'}</p>
                     </div>
                 </div>

                 {/* Contact Info */}
                 <div className="bg-white dark:bg-slate-900 rounded-xl p-4 border border-slate-100 dark:border-slate-800 space-y-3">
                     <h3 className="font-bold text-slate-800 dark:text-slate-100 text-sm">Información de Contacto</h3>
                     {selectedCustomer.phone && (
                         <div className="flex items-center justify-between">
                             ...
                             <div className="flex gap-2">
                                <a href={`tel:${selectedCustomer.phone}`} className="p-1.5 bg-slate-100 dark:bg-slate-800 rounded text-blue-500"><Phone size={14}/></a>
                                <a href={getWhatsappLink(selectedCustomer.phone)} target="_blank" className="p-1.5 bg-emerald-100 dark:bg-emerald-900/20 rounded text-emerald-500"><MessageCircle size={14}/></a>
                             </div>
                         </div>
                     )}
                     ...
                     <div className="pt-2 flex gap-2">
                        <button onClick={() => { onEdit(selectedCustomer); setSelectedCustomer(null); }} className="flex-1 py-2 text-sm bg-slate-100 dark:bg-slate-800 rounded-lg font-medium text-slate-600 dark:text-slate-300">
                            Editar
                        </button>
                     </div>
                 </div>
                 ...
                 <div>
                     ...
                     <div className="space-y-2">
                         {history.length === 0 ? (
                             ...
                             history.map(sale => (
                                 <div key={sale.id} className="bg-white dark:bg-slate-900 p-3 rounded-xl border border-slate-100 dark:border-slate-800 flex justify-between items-center">
                                     <div>
                                         <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">Compra #{sale.id.slice(0,4)}</p>
                                         <p className="text-xs text-slate-500">{new Date(sale.date).toLocaleDateString()} • {sale.items.length} items</p>
                                     </div>
                                     <div className="text-right">
                                         <p className="font-bold text-slate-800 dark:text-slate-100">{formatCurrency(sale.total)}</p>
                                         <span className="text-[10px] bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded text-slate-500">{sale.paymentMethod}</span>
                                     </div>
                                 </div>
                             ))
                         )}
            ...
            >
              <div className="flex justify-between items-start mb-2">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="font-bold text-slate-800 dark:text-slate-100">{customer.name}</h3>
                ...
                <div className="text-right">
                    <p className={`font-bold ${customer.balance < 0 ? 'text-red-500' : 'text-emerald-500'}`}>
              ...
              <div className="flex gap-2 justify-end mt-2 pt-2 border-t border-slate-50 dark:border-slate-800/50" onClick={(e) => e.stopPropagation()}>
                   <button onClick={() => onEdit(customer)} className="p-2 text-slate-400 hover:text-blue-500 dark:hover:text-blue-400">
                     <Edit2 size={18} />
                   </button>
                   <button onClick={() => onDelete(customer.id)} className="p-2 text-slate-400 hover:text-red-500 dark:hover:text-red-400">
                     <Trash2 size={18} />
                   </button>
              </div>
            </div>
