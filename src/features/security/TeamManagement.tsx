...
import { Plus, Trash2, Edit2, Shield, Save, X, Key } from 'lucide-react';
...
const TeamManagement: React.FC<TeamManagementProps> = ({
  ...
  const renderRoleBadge = (role: Role) => {
    switch (role) {
      case Role.ADMIN:
        return (
          <span className="px-2 py-0.5 bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300 rounded text-[10px] uppercase font-bold">
            Admin
          </span>
        );
      case Role.MANAGER:
        return (
          <span className="px-2 py-0.5 bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 rounded text-[10px] uppercase font-bold">
            Encargado
          </span>
        );
      default:
        return (
          <span className="px-2 py-0.5 bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-400 rounded text-[10px] uppercase font-bold">
            Vendedor
          </span>
        );
    }
        ...
        {users.map((u) => (
          ...
          >
            <div className="flex items-center gap-3">
              <div
                className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-white ${
                  u.role === Role.ADMIN ? 'bg-purple-500' : 'bg-slate-400'
                }`}
              >
            ...
            <div className="flex gap-2">
              <button
                onClick={() => openForm(u)}
                className="p-2 text-blue-500 bg-blue-50 dark:bg-blue-900/20 rounded-lg"
              >
                <Edit2 size={18} />
              </button>
              {u.id !== currentUser.id && (
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-scale-up">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <button
                type="submit"
                className="w-full bg-blue-600 text-white py-3 rounded-xl font-bold mt-4 flex items-center justify-center gap-2"
              >
