...
const FinancialAnalysis: React.FC<FinancialAnalysisProps> = ({
  ...
  const inventoryData = useMemo(() => {
    ...
    const categoryStats = Object.values(Category)
      .map((cat) => {
  ...
  const balanceData = useMemo(() => {
    ...
    const expenseComposition = Object.values(ExpenseCategory)
      .map((cat) => {
  ...
  const handleApplyInflation = () => {
    const confirmation = window.confirm(
      `¿Estás seguro de aumentar un ${inflationRate}% el precio de ${
        selectedCategoryForUpdate === 'ALL' ? 'TODOS los productos' : selectedCategoryForUpdate
      }?`
    );
        ...
        {activeTab === 'BALANCE' && (
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="bg-gradient-to-br from-slate-800 to-slate-900 dark:from-slate-800 dark:to-slate-950 rounded-2xl p-6 text-white shadow-lg relative overflow-hidden">
              ...
              <p className="text-secondary text-sm font-medium uppercase mb-1">
                Resultado Neto ({balanceData.monthName})
              </p>
              <div className="flex items-baseline gap-2">
                <h2
                  className={`text-3xl font-bold ${
                    balanceData.netProfit >= 0 ? 'text-emerald-400' : 'text-red-400'
                  }`}
                >
            ...
            <div>
              ...
              <div className="space-y-2">
                {balanceData.filteredExpenses.length === 0 ? (
                  <p className="text-secondary text-sm text-center py-4 bg-slate-50 dark:bg-slate-900/50 rounded-xl">
                    No hay gastos registrados este mes.
                  </p>
                ) : (
                  balanceData.filteredExpenses
                    .slice()
                    .reverse()
                    .map((expense) => (
                      <div
                        key={expense.id}
                        className="bg-white dark:bg-slate-900 p-3 rounded-xl border border-slate-100 dark:border-slate-800 flex justify-between items-center group"
                      >
                        <div>
          ...
          <div className="space-y-6 animate-fade-in">
                  ...
                  <div className="bg-white/10 rounded-xl p-3 border border-white/10">
                    ...
                    <div className="flex justify-between items-center text-sm">
                      ...
                      <span className="font-bold text-amber-400 text-base">
                        {formatCurrency(
                          Math.ceil((products[0].price * (1 + inflationRate / 100)) / 10) * 10
                        )}
                      </span>
                    </div>
                  </div>
            ...
            <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800">
              ...
              <div style={{ width: '100%', height: 200 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart
                    data={inventoryData.categoryStats}
                    layout="vertical"
            ...
            {/* Rankings */}
            <div>
              <h3 className="text-base font-semibold text-primary mb-3 px-1">
                Top Rentabilidad (%)
              </h3>
              <div className="space-y-2">
                {inventoryData.sortedByMarkup.slice(0, 3).map((p, idx) => (
                  <div
                    key={p.id}
                    className="bg-white dark:bg-slate-900 p-3 rounded-xl border border-slate-100 dark:border-slate-800 flex items-center justify-between"
                  >
                    <div className="flex items-center gap-3">
                      <span className="text-secondary font-mono text-sm w-4">{idx + 1}</span>
                      <div className="flex-1 min-w-0">
                        <p className="font-medium text-primary text-sm truncate w-32 sm:w-40">
                          {p.name}
                        </p>
                      </div>
                    </div>
                    <div
                      className={`px-2 py-1 rounded-lg font-bold text-xs ${getMarginColor(
                        p.markup
                      )}`}
                    >
                      {p.markup.toFixed(0)}%
