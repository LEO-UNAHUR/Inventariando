...
import { Product, BusinessIntelligence, User, IAProvider } from '@/types';
...
const AIAssistant: React.FC<AIAssistantProps> = ({
  ...
  useEffect(() => {
    if (currentUser) {
      ...
      const googleToken = settings.googleAccessToken
        ? decryptCredential(settings.googleAccessToken)
        : '';
  ...
  return (
    <div
      className="h-full flex flex-col pb-20 bg-slate-50 dark:bg-slate-950 transition-colors duration-300"
      data-tour="ai-section"
    >
        ...
        >
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1">
              ...
              >
                Proveedor IA:{' '}
                <span className="text-purple-600 dark:text-purple-400">{provider}</span>
              </p>
              {!providerReady && (
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 shadow-sm border border-slate-100 dark:border-slate-800">
              ...
              <div className="h-64 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart
                    data={data.predictions.slice(0, 5)}
                    layout="vertical"
              ...
              <div className="mt-4 space-y-2">
                {data.predictions.slice(0, 3).map((pred, idx) => (
                  <div
                    key={idx}
                    className="flex items-center justify-between text-xs border-t border-slate-100 dark:border-slate-800 pt-2"
                  >
            ...
            <div>
              ...
              <div className="space-y-3">
                {data.restockSuggestions.map((rec, idx) => (
                  <div
                    key={idx}
                    className={`p-4 rounded-xl border flex flex-col gap-2 ${getUrgencyColor(
                      rec.urgency
                    )}`}
                  >
                ...
                {data.restockSuggestions.length === 0 && (
                  <p className="text-center text-slate-400 text-sm py-4">
                    No hay sugerencias de compra urgentes por el momento.
                  </p>
                )}
              </div>
            </div>
